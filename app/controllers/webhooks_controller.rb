class WebhooksController < ApplicationController
    ...
    def stripe
        stripe_secret_key = Rails.application.config.credentials.dig(:stripe, :secret_key)
        Stripe.api_key = stripe_secret_key
        payload = request.body.read
        sig_header = request.env["HTTP_STRIPE_SIGNATURE"]
        endpoint_secret = Rails.application.config.credentials.dig(:stripe, :webhook_secret)
        event = nil

        begin
            event = Stripe::Webhook.construct_even(payload, sig_header, endpoint_secret)
        rescue JSON:ParserError => e
            status 400
            return
        rescue Stripe::SignatureVerificationError => e
            puts "Webhook signature verification error"
            status 400
            return
        end
        ...
        when 'checkout.session.completed'
            ...
            line_items['data'].each do |item|
                product = Stripe::Product.retrieve(item["item"]["product"])
